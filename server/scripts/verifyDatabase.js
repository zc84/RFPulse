...
const requiredTables = [
  ...
  'ai_findings',
  'knowledge_sections',
  'endpoint_visual_plans',
  'endpoint_visual_idempotency',
  'endpoint_visual_usage_budget',
  'endpoint_visual_provider_leases',
];
...
async function verifyDatabase() {
  ...
  const functionCheck = await query(
    ...
    [[
      'reserve_endpoint_visual_budget',
      'claim_endpoint_visual_provider_lease',
    ]],
  );
  const foundFunctions = new Set(functionCheck.rows.map(row => row.proname));
  const missingFunctions = [
    'reserve_endpoint_visual_budget',
    'claim_endpoint_visual_provider_lease',
  ].filter(name => !foundFunctions.has(name));
  if (missingFunctions.length > 0) {
    throw new Error(
      `Database verification failed; missing functions: ${missingFunctions.join(', ')}`
    );
  }

  console.log(
    `Database verification passed (${requiredTables.length} required tables, visual functions, and migrated session columns).`
  );
}
